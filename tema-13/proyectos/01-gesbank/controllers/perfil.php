...
class Perfil extends Controller
{
    ...
    public function valperfil()
    {
        ...
        if (!empty($errores)) {
            ...
            # Envío de correo electrónico de modificación
            $this->sendEmailModification($user->email, $name);
    ...
    # Envío de correo electrónico de modificación
    private function sendEmailModification($email, $name)
    {
        ...
        try {
            ...
            $mail->Subject = 'Perfil modificado correctamente';
            $mail->Body = 'Tu perfil ha sido modificado con éxito. Los nuevos detalles son: <br>' .
                '<b>Nombre</b>: ' . $name . '<br>' .
                '<b>Email</b>: ' . $email . '<br>';
            $mail->send();
        } catch (Exception $e) {
    ...
    public function valpass()
    {
        ...
        if (!empty($errores)) {
            ...
            $this->sendEmailPasswordChange($user->email, $user->password);
    ...
    # Envío de correo electrónico de cambio de contraseña
    private function sendEmailPasswordChange($email, $password)
    {
        // Configuración de PHPMailer
        $mail = new PHPMailer(true);
        try {
            ...
            $remitente = SMTP_USER;
            $mail->isHTML(true);
            $mail->setFrom($remitente, 'Gesbank');
    ...
    public function delete()
    {
        ...
        if (!isset($_SESSION['id'])) {
            ...
            # Envío de correo electrónico
            $this->sendEmailProfileDeletion($user->email, $user->name);
    ...
    # Envío de correo electrónico de eliminación de perfil
    private function sendEmailProfileDeletion($email, $name)
    {
        // Configuración de PHPMailer
        $mail = new PHPMailer(true);
        try {
